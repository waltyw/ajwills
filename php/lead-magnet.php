<?php
/**
 * AJ Wills & Estate Planning — Lead Magnet Download Endpoint
 * "The 10 Essential Things Every UK Will Should Include"
 * Captures Name, Email, Phone (optional) before granting download access.
 */
...
define('GUIDE_URL',       '/guides/10-essential-things-every-uk-will-should-include.pdf');
...
function respond(bool $s, string $m, int $c = 200, array $extra = []): void {
    http_response_code($c);
    header('Content-Type: application/json; charset=utf-8');
    echo json_encode(array_merge(['success' => $s, 'message' => $m], $extra));
    exit;
}

// Honeypot
if (!empty($_POST['website_url'])) respond(true, 'OK', 200, ['downloadUrl' => GUIDE_URL]);
...
// Log to CSV
$logFile = __DIR__ . '/guide-downloads.csv';
if (!file_exists($logFile)) {
    file_put_contents($logFile, '"Date","Name","Email","Phone","IP"' . "\n", FILE_APPEND | LOCK_EX);
}
$row = implode(',', array_map(fn($v) => '"' . str_replace('"', '""', $v) . '"', [
    $receivedAt, $name, $email, $phone, $ip
])) . "\n";
...
<div class="w">
  <div class="hd"><h1 style="margin:0;font-size:1.15rem">New Guide Download</h1><p style="margin:4px 0 0;color:rgba(255,255,255,.65);font-size:.8rem">{$receivedAt}</p></div>
  <div class="bd">
    ...
    <div class="field"><div class="lbl">Guide</div><div class="val">10 Essential Things Every UK Will Should Include</div></div>
  </div>
</div>
</body></html>
HTML;
...
@mail(RECIPIENT_EMAIL, "New Guide Download: {$name}", $html, $headers);

respond(true, 'Thank you! Your guide is ready to download.', 200, ['downloadUrl' => GUIDE_URL]);
